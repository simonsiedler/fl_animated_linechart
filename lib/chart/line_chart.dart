...
class LineChart {
  ...
  LineChart(
    ...
    String formatDayMonth = 'dd.MM.yy',
  ...
  factory LineChart.fromDateTimeMaps(List<Map<DateTime, double>> series, List<Color> colors, List<String> units,
      {FontWeight tapTextFontWeight, TextStyle axisTextStyle}) {
    ...
    return LineChart(convertFromDateMaps.left, convertFromDateMaps.right,
        tapTextFontWeight: tapTextFontWeight, axisTextStyle: axisTextStyle);
  }
  ...
  void initialize(double widthPX, double heightPX) {
    ...
    for (int axisIndex = 0; axisIndex < indexToUnit.length; axisIndex++) {
      ...
      for (int c = 0; c <= (stepCount + 1); c++) {
        ...
        TextSpan span = new TextSpan(style: axisTextStyle, text: axisValueString);
        TextPainter tp =
            new TextPainter(text: span, textAlign: TextAlign.right, textDirection: TextDirectionHelper.getDirection());
        tp.layout();
    ...
    for (int c = 0; c <= (stepCount + 1); c++) {
      ...
      TextSpan span = new TextSpan(style: axisTextStyle, text: _formatDateTime(tick, duration));
      TextPainter tp =
          new TextPainter(text: span, textAlign: TextAlign.right, textDirection: TextDirectionHelper.getDirection());
      tp.layout();
